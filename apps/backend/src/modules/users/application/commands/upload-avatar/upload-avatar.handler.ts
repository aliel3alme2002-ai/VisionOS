...
export class UploadAvatarHandler
  implements BaseCommandHandler<UploadAvatarCommand, UserResponseDto>, ICommandHandler<UploadAvatarCommand>
{
  constructor(@Inject('IUserRepository') private readonly repository: IUserRepository) {}

  async execute(command: UploadAvatarCommand): Promise<UserResponseDto> {
    ...
    user.uploadAvatar(command.storageObjectId, command.thumbnailUrl);
